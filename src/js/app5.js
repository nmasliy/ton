...
window.addEventListener('DOMContentLoaded', function() {
    ...
    function initMenu() {
        ...
        const $headerOverlay = document.querySelector('.header__overlay');
				const $bodyios = document.querySelector('body');
				const $signup_triger = document.querySelector('.signup-triger');
        ...
        const checkScreenWidth = () => {
            ...
            // Активируем меню только на экранах <= 850
            if (window.innerWidth <= MOBILE_MENU_BREAKPOINT && !isInit) {
                ...
                $headerOverlay.addEventListener('click', closeMenu);
								if ($signup_triger) {
                                    $signup_triger.addEventListener('click', closeMenu);
                                }
            } else {
        ...
        function openMenu() {
            ...
            $html.classList.add('overflow-hidden');
						$bodyios.classList.add('overflow-hidden');
        ...
        function closeMenu() {
            ...
            $html.classList.remove('overflow-hidden');
						$bodyios.classList.remove('overflow-hidden');
    ...
    function initFilterMenu() {
        ...
        const $filterOverlay = document.querySelector('.filter__overlay');
				// const $ioslock = document.querySelector('#page');
				const $bodyios = document.querySelector('body');
        const TRANSITION_DELAY = 400; 
        ...
        if ($filter) {
            ...
            function closeFilter() {
                ...
                $html.classList.remove('overflow-hidden');
								// $ioslock.classList.remove('ioslock');
								$bodyios.classList.remove('overflow-hidden');
        ...
        if ($filterMoreBtn) {
            $filterMoreBtn.addEventListener('click', function(e) {
                // $filterMoreBtn.classList.add('hidden');
								// $filterMoreBtn.textContent('Show less');
								this.textContent = this.textContent === 'Show all' ? 'Show less' : 'Show all';
								// e.target.closest('.filter__categories').classList.add('active');
                e.target.closest('.filter__categories').classList.toggle('active');
            })
        }
    }
